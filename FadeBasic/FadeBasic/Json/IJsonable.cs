...
namespace FadeBasic.Json
{
    ...
    {
        void ProcessJson<T>(ref T op) where T : IJsonOperation;
    }
    ...
    public struct JsonWriteOp : IJsonOperation
    {
        ...
        public void IncludeField<T>(string name, ref T fieldValue) where T : IJsonable, new()
        {
            ...
            else
            {
                ...
                fieldValue.ProcessJson(ref subOp);
                _sb.Append(JsonConstants.CLOSE_BRACKET);
            }
    ...
    public struct StreamingJsonReadOp : IJsonOperation
    {
        ...
        public StreamingJsonReadOp(string json, int objStart)
        {
            ...
            _fieldCount = 0;
            ObjEnd = 0;
            _fields = ArrayPool<int>.Shared.Rent(InitialCapacity * FieldStride);
            BuildIndex(objStart);
        }
        ...
        public void Return()
        {
            if (_fields != null)
            {
                ArrayPool<int>.Shared.Return(_fields);
                _fields = null;
            }
        }
        ...
        private void BuildIndex(int i)
        {
            ...
            while (i < _json.Length && _json[i] != '}')
            {
                ...
                {
                    var newSize = _fields.Length * 2;
                    var grown = ArrayPool<int>.Shared.Rent(newSize);
                    Array.Copy(_fields, grown, _fieldCount * FieldStride);
                    ArrayPool<int>.Shared.Return(_fields);
                    _fields = grown;
                }
        ...
        {
            var sign = 1;
            if (i < _json.Length && _json[i] == '-') { sign = -1; i++; }
            var value = 0;
            while (i < _json.Length && _json[i] >= '0' && _json[i] <= '9')
                value = value * 10 + (_json[i++] - '0');
            return sign * value;
        }
