...
public partial class FadeDebugAdapter
{
    ...
    protected override void HandleVariablesRequestAsync(IRequestResponder<VariablesArguments, VariablesResponse> responder)
    {
        ...
        Attempt();
        void Attempt(bool canExpand=true)
        {
            if (!db.TryGetEntry(responder.Arguments.VariablesReference, out var entry))
            {
                _logger.Log($"[WARN] invalid variables requested. id=[{responder.Arguments.VariablesReference}] ");
                if (!canExpand)
                {
                    responder.SetError(new ProtocolException("Unable to inspect variables"));
                    return;
                }
                _session.RequestVariableInfo(responder.Arguments.VariablesReference, foundScope =>
                {
                    _logger.Log(
                        $"[VB] found scope for id=[{responder.Arguments.VariablesReference}], {foundScope.Count}");
                    foreach (var dbgScope in foundScope)
                    {
                        var scope = db.AddScope(1, dbgScope);
                    }
                    Attempt(false);
                });
                return;
